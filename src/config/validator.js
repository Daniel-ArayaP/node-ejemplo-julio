...
module.exports = (routeName) => {
    switch (routeName) {
        case 'signUp':
            return [
                check('nombre').not().isEmpty().withMessage('Name is Required'),
                check('telefono').not().isEmpty().withMessage('Phone is Required'),
                check('telefono').isFloat().withMessage('Telefono must be a Number'),
